...
class UDPHandler(socketserver.BaseRequestHandler):   
    ...
    def sendResponse(self,code):
        ...
        data.append("")
        text = "\r\n".join(data)
        self.socket.sendto(str.encode(text),self.client_address)
        showtime()
        
        
        #logging.info("<<< %s tu je nieco response" % data[0])
    ...
    def processRegister(self):
        ...
        logging.info("Nova registracia: %s - Adresa: %s" % (fromm,contact))
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                text = "\r\n".join(data)
                socket.sendto(str.encode(text) , claddr)
                showtime()
                #logging.info("<<< %s Invite na " % data[0])
                logging.info("Zaciatok hovoru: " + destination + " na " +  origin)     
                logging.debug("---\n<< server send [%d]:\n%s\n---" % (len(text),text))
            else:
                self.sendResponse("480 Ucastnik momentalne nedostupny")
        else:
    ...
    def processAck(self):
        ...
        if len(destination) > 0:
            ...
            if  destination in registrar:
                ...
                socket.sendto(str.encode(text),claddr)
                showtime()
                #logging.info("<<< %s ACk hovoru  destination" % data[0])     
    ...
    def processNonInvite(self):
        ...
        if len(destination) > 0:
            ...
            if  destination in registrar and self.checkValidity(destination):
                ...
                socket.sendto(str.encode(text) , claddr)
                showtime()
                logging.info("Ukoncenie hovoru: " + destination + " a " +  origin)
                logging.debug("---\n<< server send [%d]:\n%s\n---" % (len(text),text))    
            else:
    ...
    def processCode(self): 
        ...
        if len(origin) > 0:
            ...
            if  origin in registrar:
                ...
                socket.sendto(str.encode(text),claddr)
                showtime()
                #logging.info("<<<  %s process code" % data[0])
                if(data[0] == 'SIP/2.0 180 Ringing'):                  
                    logging.info("Zvonenie: " +  origin + "  " + claddr[0] + ":" + str(claddr[1]) )
                elif(data[0] == 'SIP/2.0 200 Ok'):   
                    logging.info("Uspesne zacaty/skonceny: " +  origin + "  " + claddr[0] + ":" + str(claddr[1]))
                elif(data[0] == 'SIP/2.0 603 Decline'):   
                    logging.info("Hovor odmietnuty: "+  origin + "  " + claddr[0] + ":" + str(claddr[1]))
                elif(data[0] == 'SIP/2.0 486 Busy here'):   
                    logging.info("Hovor nezdvihnuty: "+  origin + "  " + claddr[0] + ":" + str(claddr[1]))
    ...
    def handle(self):
        ...
        if rx_request_uri.search(request_uri) or rx_code.search(request_uri):
            showtime()
            #logging.info(">>> %s tu je handle, on sa meni" % request_uri)
            logging.debug("---\n>> server received [%d]:\n%s\n---" %  (len(data),data))
